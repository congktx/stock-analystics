...
def get_historical_data(ticker, target_timestamp_ms):
    ...
    try:
        ...
        base_params = {
            ...
            "limit": 1000 # Số lượng record mỗi trang (tùy chỉnh theo API server của bạn)
        }
        
        all_docs = []
        
        # --- BƯỚC 1: Gọi Page 1 trước để lấy Meta Data (page_count) ---
        print(f"--- Đang tải dữ liệu {ticker} (Page 1)...")
        base_params["page"] = 1
        ...
        print(f"-> Tìm thấy tổng cộng {page_count} trang.")
...
def predict_stock_pytorch(ticker, timestamp_ms):
    ...
    if next_candle:
        ...
        next_date = datetime.fromtimestamp(next_candle['t']/1000).strftime('%Y-%m-%d')
    else:
        next_date = "N/A"
    ...
    if actual_return is not None:
        print(f"📈 Thực tế (T+1)  : {actual_return:.6f} ({actual_return*100:+.2f}%)")
        ...
        if trend_pred == trend_real:
            ...
            print(f"❌ Sai xu hướng (Dự báo {trend_pred} nhưng thực tế {trend_real})")
    else:
...
if __name__ == "__main__":
    # Test với timestamp giả định (1 ngày trong quá khứ để thấy Actual)
    # Ví dụ: lấy timestamp của 1 tuần trước
    predict_stock_pytorch("VTYX", 1762794000000)